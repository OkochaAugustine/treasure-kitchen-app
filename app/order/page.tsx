...
export default function OrderPage() {
  ...
  const [swallowQty, setSwallowQty] = useState<Record<SwallowType, number>>({
    eba: 0,
    fufu: 0,
    semovita: 0,
  });
  ...
  const handleProceedPayment = async () => {
    const orderData = {
      ...
      plates,
      swallow: swallowQty,
      meattype: meatType,     // ✅ matches DB column
      meatqty: meatQty,       // ✅ matches DB column
      totalprice: totalPrice, // ✅ matches DB column
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/orderz/create", {
        method: "POST",
      ...
      if (res.ok) {
        ...
        setSoup("");
        setPlates(1);
        setSwallowQty({ eba: 0, fufu: 0, semovita: 0 });
        setMeatType("");
        setMeatQty(0);
        onClose();
      } else {
    ...
    <Box position="relative" minH="100vh" py={10} px={5} bgGradient="linear(to-br, orange.50, pink.50)">
      ...
      <Box maxW="600px" mx="auto" bg="white" borderRadius="2xl" shadow="2xl" p={8}>
        ...
        <VStack spacing={6}>
          ...
          <Card bg={cardBg} shadow="lg" borderRadius="2xl" w="full">
            <CardBody>
              ...
              <InputGroup>
                ...
                <Input value={phone} onChange={(e) => setPhone(e.target.value)} size="lg" placeholder="Enter phone" />
              </InputGroup>
            </CardBody>
          </Card>
          ...
          <Card bg={cardBg} shadow="lg" borderRadius="2xl" w="full">
            <CardBody>
              <Text fontSize="lg" fontWeight="bold" mb={2}>📍 Delivery Location</Text>
              <Input value={location} onChange={(e) => setLocation(e.target.value)} size="lg" placeholder="Enter location" />
            </CardBody>
          </Card>
          ...
          <Card bg={cardBg} shadow="lg" borderRadius="2xl" w="full">
            <CardBody>
              <Text fontSize="2xl" fontWeight="bold" mb={3}>🥣 Choose Soup</Text>
              <Select
                placeholder="Select soup"
                value={soup}
                onChange={(e) => setSoup(e.target.value as SoupType)}
                size="lg"
                icon={<ChevronDownIcon />}
              >
                {Object.keys(PRICES.soups).map((key) => (
                  <option key={key} value={key}>
                    {key.toUpperCase()} - ₦{PRICES.soups[key as SoupType].toLocaleString()}
                  </option>
                ))}
              </Select>

              <Text fontWeight="600" mt={3}>Number of Plates</Text>
              <NumberInput min={1} value={plates} onChange={(v) => setPlates(Number(v))}>
                <NumberInputField />
              </NumberInput>
            </CardBody>
          </Card>
              ...
              <VStack spacing={3} align="stretch">
                {Object.keys(swallowQty).map((key) => (
                  <Box key={key} display="flex" justifyContent="space-between" alignItems="center">
                    <Text fontWeight="600">
                      {key.toUpperCase()} - ₦{PRICES.swallow[key as SwallowType].toLocaleString()}
                    </Text>
                    <NumberInput
                      min={0}
                      value={swallowQty[key as SwallowType]}
                      width="100px"
                      onChange={(v) =>
                        setSwallowQty((prev) => ({ ...prev, [key]: Number(v) }))
                      }
                    >
                      <NumberInputField />
                    </NumberInput>
          ...
          <Card bg={cardBg} shadow="lg" borderRadius="2xl" w="full">
            <CardBody>
              <Text fontSize="2xl" fontWeight="bold" mb={3}>🍗 Choose Meat</Text>
              <Select
              ...
              >
                {Object.keys(PRICES.meats).map((key) => (
                  <option key={key} value={key}>
                    {key.toUpperCase()} - ₦{PRICES.meats[key as MeatType].toLocaleString()}
                  </option>
                ))}
              </Select>
              ...
              <NumberInput
                min={0}
                value={meatQty}
                onChange={(v) => setMeatQty(Number(v))}
                mt={3}
              >
                <NumberInputField placeholder="Number of pieces" />
              </NumberInput>
            </CardBody>
          </Card>
